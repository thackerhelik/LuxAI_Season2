"""
Implementation of RL agent. Note that luxai_s2 and stable_baselines3 are packages not available 
during the competition running (ATM)
"""
...
class CustomEnvWrapper(gym.Wrapper):
    def __init__(self, env: gym.Env) -> None:
        """
        Adds a custom reward and turns the LuxAI_S2 environment into a single-agent environment 
        for easy training
        """
        super().__init__(env)
    ...
    def step(self, action):
        ...
        for k in opp_factories.keys():
            ...
            # set enemy factories to have 1000 water to keep them alive the whole around and treat 
            # the game as single-agent
